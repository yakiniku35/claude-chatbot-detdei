...
# 設定頁面
st.set_page_config(page_title="DEI 聊天機器人", page_icon="🤖", layout="centered")
...
def init_langchain_groq():
    ...
    return ChatGroq(model=selected_model, api_key=api_key, temperature=0.7)
...
async def tool_node(state: AgentState, search_tool):
    """執行搜尋工具"""
    tool_calls = (
        state["messages"][-1].tool_calls if hasattr(state["messages"][-1], "tool_calls") else []
    )
    tool_messages = []
...
def detect_language(text):
    ...
    has_japanese = any(
        "\u3040" <= char <= "\u309f" or "\u30a0" <= char <= "\u30ff" for char in text
    )
...
def chat(client, messages, use_search=True):
    ...
    if use_search and last_msg and should_search(last_msg["content"]):
        ...
        if results:
            search_context = "\n\n參考網路資訊：\n" + "\n".join(
                [f"• {r.get('title', '')}: {r.get('body', '')[:100]}..." for r in results[:2]]
            )
    ...
    if prompts_data.get("executive_orders"):
        ...
        for order in prompts_data["executive_orders"]:
            executive_orders_parts.append(
                f"• **{order.get('title', '')}**: {order.get('description', '')}"
            )
...
"""
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="openai/gpt-oss-120b", messages=msgs, temperature=0.7, max_tokens=2500
        )
...
with st.sidebar:
    ...
    uploaded = st.file_uploader(
        "📎 上傳檔案", type=["pdf", "docx", "txt"], help="支援 PDF、Word、TXT 格式"
    )
    ...
    if st.button("🗑️ 清除對話", use_container_width=True):
        ...
        st.session_state.messages = [
            {"role": "assistant", "content": "對話已清除！😊 有什麼我可以幫你的嗎？"}
        ]
        st.session_state.file_processed = set()
        st.rerun()
...
if prompt := st.chat_input("輸入訊息..."):
    ...
    with st.chat_message("assistant"):
        ...
        use_agent = (
            ...
            and st.session_state.get("search", True)
        )
...
{executive_orders_text}{policies_text}"""
                ...
                try:
                    ...
                    # 檢查是否使用了搜尋
                    if "tavily" in str(response).lower() or any(
                        keyword in prompt.lower() for keyword in ["最新", "latest", "2024", "2025"]
                    ):
                        response += "\n\n🌐 *此回覆使用智能搜尋*"
                except Exception as e:
            ...
            with st.spinner("思考中..."):
                response = chat(
                    client, st.session_state.messages, st.session_state.get("search", True)
                )
